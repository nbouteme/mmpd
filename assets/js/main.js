...
{
  var musicelem = $('<div>');
  var musicbox  = $('<div>').appendTo(musicelem);
  var box       = $('<div>').appendTo(musicbox);
  var boximg    = $('<img>').appendTo(box);
  var musicinfo = $('<div>').appendTo(musicbox);

  musicelem.addClass('col-lg-2 col-md-3 col-sm-4 col-xs-12');
  musicbox.addClass('music-box');
  box.addClass('box');
  boximg.addClass('dummy-content');
  boximg.attr('src', '/mpd/songs/' + music.coverId + '/cover');
  musicinfo.addClass('music-info');

  $('<h4>').appendTo(musicinfo).addClass('h4').html(music.title);
  $('<h6>').appendTo(musicinfo).addClass('h6').html(music.artist);
  $('<hr>').appendTo(musicinfo);
  return musicelem;
}
  ...
  timeRefresh.done(function(val)
                   {
                     if(!$('#time-slider').prohibit)
                       $('#time-slider').slider('value', 100 * val.currentSec / val.durationSec);
  ...
  $.ajax(
    {
      ...
      dataType: 'json'
    }).done(function(val)
            {
              ...
              if(sub.length != 0)
              {
                $('#music-list > div').each(function(i)
                                            {
                                              url = $(this).find('img').attr('src');
                                              for(var i = 0; i < sub.length; ++i)
                                                if('/mpd/songs/' + sub[i].coverId + '/cover' == url)
                                                {
                                                  $(this).remove();
                                                  return;
                                                }
                ...
                $.ajax(
                  {
                    ...
                    dataType: 'json'
                  }).done(function(val)
                          {
                            ...
                            $('#current-playing').attr('src', '/mpd/songs/' + val.coverId + '/cover');
                          });      
              }
            });
  refreshTime();
