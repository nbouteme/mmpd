...
function makePlItemElem(music, pl) {
    return makeGenericItem(
        ...
        music.title, !music.artist ? 'Inconnu' : music.artist,
        makePlItemMenu, [music, pl]
    ).data('pl', pl);
}
...
function loadPlaylists(src) {
    loadItems('playlists', makePlElem, function() {
        $('#music-list').append(makePlElem(''));
    });
}
...
function updatePlayer() {
    ...
    $.ajax({
        ...
        if (((val.state == "pause" || val.state == "stop") && $("#ctrl-btn").hasClass('fa-pause')) || (val.state == "play" && $("#ctrl-btn").hasClass('fa-play')))
...
function onMoveOverTimeSlider(event) {
    ...
    tri.css({
        ...
        zIndex: 999,
        bottom: "72px",
        left: (p > maxWidth - 9 ? maxWidth - 9 : p < 9 ? 9 : p) - tri.outerWidth() / 2 + "px"
    });
            ...
            children: [{
                ...
                children: [{
                    ...
                    children: [{
                        ...
                        events: [{
                            click: function() {
                                sessionStorage.setItem('refused', true);
                                modalAsk.remove();
                            }
                        }]
                    }, {
